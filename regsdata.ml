...
module Regsdata : sig
  ...
  let rec fill_array (l:(int*int*int) list) (a_cp: int Array.t) (a_clk: int Array.t) : unit =
    match l with
    | [] -> ()
    | (i,cp,clk)::l' ->
        ...
        fill_array l' a_cp a_clk

  (* array is more recent than list: list cannot overwrite array *)
  let compress_arrays_and_list (a_cp: int Array.t) (a_clk: int Array.t) (l: (int * int * int) list):
       int Array.t * int Array.t =
    fill_array l a_cp a_clk;
    (a_cp, a_clk)

  let compress_arrays_and_lists (a_cp: int Array.t) (a_clk: int Array.t) (l_old: (int * int * int) list) (l_new: (int * int * int) list): 
      int Array.t * int Array.t =
    fill_array l_new a_cp a_clk;
    fill_array l_old a_cp a_clk;
    (a_cp, a_clk)
  ...
  let compress (regs_size: p) (t1: t) (t2: t): t = 
    ...
    | Complete a1, Incomplete l2 -> (* incorporate l2 in a1 *)
      if l2.size = 0 then t1
      else if l2.size = 1 then (
        let (i, new_cp, new_clk) = List.hd l2.l in
        if (new_cp <> -1) then a1.a_cp.(i) <- new_cp;
        if (new_clk <> -1) then a1.a_clk.(i) <- new_clk;
        t1
      ) else (
        let (a_cp, a_clk) = compress_list_and_arrays a1.a_cp a1.a_clk l2.l in
        Complete({a_cp=a_cp; a_clk=a_clk}) 
      )
    | Incomplete l1, Complete a2 -> (* incorporate l1 into a2 *)
      let (a_cp, a_clk) = compress_arrays_and_list a2.a_cp a2.a_clk l1.l in
      Complete({a_cp=a_cp; a_clk=a_clk}) 
    | Incomplete l1, Incomplete l2 ->
      ...
      if size >= regs_size then (
        let (a_cp, a_clk) = compress_arrays_and_lists (Array.make regs_size (-1)) (Array.make regs_size (-1)) l1.l l2.l in
        Complete({a_cp=a_cp; a_clk=a_clk})
      ) else 
  ...
  let to_arrays (size: int) (v: t) : int Array.t * int Array.t =
    ...
    | Incomplete l -> compress_arrays_and_list (Array.make size (-1)) (Array.make size (-1)) l.l
